...
API_KEY = "6050692509:AAFX2ND0ZYFcWQkO-gq4lDkY0GmAD8O-RWY"
...
# secret = "11222224444445555555"
# bot = telepot.Bot('6050692509:AAFX2ND0ZYFcWQkO-gq4lDkY0GmAD8O-RWY')
# bot.setWebhook("https://twobottles.pythonanywhere.com/{}".format(secret), max_connections=1)
...
@app.route('/api/v1/nostra/forecast', methods=['GET'])
def getTokenChartData():

    query_parameters = request.args

    ca = query_parameters.get('ca')
    dateFrom = query_parameters.get('dateFrom')
    dateTo = query_parameters.get('dateTo')
    forecastType = query_parameters.get('forecastType')
    timeFrameInMinutes = query_parameters.get('tfm')
    cid = query_parameters.get('cid')
    response = flask.jsonify(prophet(ca,dateFrom,dateTo,forecastType,timeFrameInMinutes,cid));    
    response.headers.add('Access-Control-Allow-Origin', '*')

    return response;

@app.route('/api/v1/nostra/token-detail', methods=['GET'])
def getTokenMetadata():
    ...
    return getTokenDetails(ca);


@app.route('/api/v1/nostra/check', methods=['GET'])
def get():
...
@app.route('/api/v1/nostra/trends', methods=['GET'])
def trends():
...
def getToken():
  ...
  keys = [
    "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",
    "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",
    "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"
  ]
  return keys[index]
